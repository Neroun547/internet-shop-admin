    ...
    <v-card class="pa-5">
      <v-form>
        ...
        <v-card-actions class="mt-5">
          <v-spacer></v-spacer>
          <v-btn class="bg-yellow-accent-4" @click="editProduct"
            >Оновити товар</v-btn
          >
        </v-card-actions>
      </v-form>
...
async function getProductById() {
  const data = (await Api.get(`/products/${route.params.id}`)).data;

  productPrice.value = data.price;
  productName.value = data.name;
  productDescription.value = data.description;
  selectedRubric.value = data.rubric_id;
  selectedRubricType.value = data.type;
  productStatus.value = Boolean(data.available);
  productNum.value = data.num;

  preloadImgSrc.value = data.images.map(function (img) {
    return {
...
async function editProduct() {
  ...
  formData.append("num", productNum.value);

  const api = await fetch(`${API_URL}/api/admin/products/${route.params.id}`, {
    method: "PUT",
    headers: {
